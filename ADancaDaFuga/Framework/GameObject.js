function GameObject(position, rotation){
	...
	this.setBoundingBoxDimensions = function(width, height){
		...
		if (boundingBox == null) {
			if (anchorPoint != null) {
				boundingBox = new BoundingBox(position.x + anchorPoint.x + width / 2, position.y + anchorPoint.y + height / 2, width, height);
			}
			else{
				boundingBox = new BoundingBox(position.x + width / 2, position.y + height / 2, width, height);
			}
		}
		else{
